...
def raw_overlap(list1, list2, depth):
    ...
    # Returns common values and length of both sets
...
def agreement(list1, list2, depth):
    ...
    """
    # This one is a little bit different from the original one.
    len_intersection, len_set1, len_set2 = raw_overlap(list1, list2, depth)
    return 2 * len_intersection / (len_set1 + len_set2) # equation 28
...
def weight_rbo_d_p(d,p):
    '''see equation 11 of the original paper'''
    w=(1-p**(d-1)+
       ((1-p)/p)*d*(
            np.log(1/(1-p))-
            sum((p ** i)/i for i in np.arange(1,d,1))
            )
        )
    return w

def plotting_weight_wrt_p(d):
    weight_df=pd.DataFrame()
    p_list=np.arange(0.01,1.0,0.01)
    for p in p_list:
        weight_df=weight_df.append({'p':p,'w':weight_rbo_d_p(d,p)},ignore_index=True)
    weight_df.set_index('p', inplace=True)
    plt.plot(weight_df['w'])
    return weight_df

def rbo_at_k_normalised_w(list1, list2, p, depth=None):
    weights=[]
    depth = min(len(list1), len(list2)) if depth is None else depth
    for i in range(depth):
        weights.append((p ** i)*(1-p))
    norm_weights=[float(w)/sum(weights) for w in weights]
    d_a = enumerate(cumulative_agreement(list1, list2, depth))
    df=pd.DataFrame()
    i=0
    for (d,a) in d_a:
        df=df.append({'d':d+1,'a':a,'w_norm':norm_weights[i],'w_org_paper':weights[i]},ignore_index=True)
        i=i+1
    df['rbo_norm']=df['a']*df['w_norm']
    df['rbo_org']=df['a']*df['w_org_paper']
    return df['rbo_norm'].sum()
