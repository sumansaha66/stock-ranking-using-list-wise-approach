# -*- coding: utf-8 -*-
"""
Created on Mon Sep  6 11:16:41 2021

@author: suman
"""
# The main target of this stuy is to use different stock market graphs for
# stock movement prediction. Previously, we used only the static graphs. What if
# we change it to some other graphs. For example, how will it perform if we use a
# correlation graph or a mutual information graph?
...
class MyModel(Model):
  def __init__(self, nCom, W_mask, rel_mask, inner_prod, flat,rel_encoding,units = 0):
...
class ReRaLSTM:
    def __init__(self, data_path, market_name, tickers_fname, relation_name,
        ...
        elif self.relation_name=='pearson_corr':
            # Calculate a correlation matrix based on gt_data price. Use training data only.
            self.corr_data=pd.DataFrame(self.gt_data[:,:self.valid_index].T).corr(method='pearson').to_numpy()            
        elif self.relation_name=='spearman_corr':
            self.corr_data=pd.DataFrame(self.gt_data[:,:self.valid_index].T).corr(method='spearman').to_numpy()
        elif self.relation_name=='mantegna_corr':
            self.corr_data_temp=pd.DataFrame(self.gt_data[:,:self.valid_index].T).corr(method='pearson').to_numpy()
            self.corr_data=np.sqrt(2*(1-self.corr_data_temp))
        # Calculate a mutual information matrix based on gt_data price. Use training data only.
        elif self.relation_name=='mi':
            self.mi_data_half=np.zeros([self.gt_data.shape[0],self.gt_data.shape[0]]).astype('float32')
            for i in range(self.gt_data.shape[0]):
                for j in range(self.gt_data.shape[0]):
                    if i>=j:
                        self.mi_data_half[i,j]=mutual_info_regression(self.gt_data[i,:self.valid_index].reshape(-1,1),
                                         self.gt_data[j,:self.valid_index],n_neighbors=4)
                    if i==j:
                        self.mi_data_half[i,j]=self.mi_data_half[i,j]/2
            self.mi_data=self.mi_data_half+self.mi_data_half.T
            self.mi_data=self.mi_data/self.mi_data.max()
                    
        if self.relation_name in ['pearson_corr','spearman_corr','mantegna_corr']:
            '''I am changing rel_encoding to the correlation matrix'''
            '''The adjacency matrix should be symmetric. It should also have three dimensions'''
            '''The rel_mask should have all 0 as I am using all relations'''
            self.rel_encoding=self.corr_data.reshape(self.corr_data.shape[0],self.corr_data.shape[1],1) # 2d to 3d
            self.rel_mask=np.zeros([self.rel_encoding.shape[0],self.rel_encoding.shape[1]])
        elif self.relation_name in ['mi']:
            self.rel_encoding=self.mi_data.reshape(self.mi_data.shape[0],self.mi_data.shape[1],1) # 2d to 3d
            self.rel_mask=np.zeros([self.rel_encoding.shape[0],self.rel_encoding.shape[1]])
        ...
        self.model = MyModel(self.numCompany, self.W_mask, self.rel_mask, 
    ...
    def train(self):
        ...
        def train_step(Feature, base_price, ground_truth, mask):
          with tf.GradientTape() as tape:
            ...
            return_ratio = tf.divide(tf.subtract(prediction, base_price), base_price) #(1026,1) tensor
          ...
          # What is the purpose of using trainable_variables? Did we define the 
          # trainable variables while  creating the model?
        ...
        for epoch in range(self.epochs):
            ...
            for j in range(self.valid_index - self.parameters['seq'] -
                ...
                '''new code start'''
                rel_weight, train_cur_loss, train_cur_reg_loss, cur_rank_loss, cur_rr= train_step(
            ...
            print('Train Loss:',
                  ...
                  tra_reg_loss.numpy() / (self.valid_index - self.parameters['seq'] - self.steps + 1),
                  tra_rank_loss.numpy() / (self.valid_index - self.parameters['seq'] - self.steps + 1))
            ...
            print('Valid Loss:',
                          ...
                          val_reg_loss.numpy() / (self.test_index - self.valid_index),
                          val_rank_loss.numpy() / (self.test_index - self.valid_index))
            ...
            print('Test loss:',
                          ...
                          test_reg_loss.numpy() / (self.trade_dates - self.test_index),
                          test_rank_loss.numpy() / (self.trade_dates - self.test_index))
            ...
            if epoch%10==0:
                ...
                self.df_loss=make_df_loss(self, epoch, cur_valid_perf, cur_test_perf, tra_loss, tra_reg_loss,
                                          val_loss, test_loss)
                self.df_loss.to_csv('df_loss_feng_modified_'+self.loss_name+'_'+self.market_name+'_'+
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rn', '--rel_name', type=str,
                        default='mi',
                        help='relation type: sector_industry,wikidata,pearson_corr,spearman_corr,mantegna_corr,mi')
    parser.add_argument('-ip', '--inner_prod', type=int, default=1)
    parser.add_argument('-depth', type=int, default=5)
    parser.add_argument('-epoch_num', type=int, default=15)
    args = parser.parse_args()
    
    # Test ticker contains only first 20 stocks
    if args.t is None:
    ...
    W_mask = RR_LSTM.W_mask
    rel_mask=RR_LSTM.rel_mask
    pred_all = RR_LSTM.train() 
    trainedModel = pred_all[0]
    weights = trainedModel.get_weights()
    df_loss=RR_LSTM.df_loss
    df_loss.to_csv('df_loss_feng_modified_'+RR_LSTM.loss_name+'_'+RR_LSTM.market_name+'_'+
